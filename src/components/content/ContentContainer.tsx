...
export function ContentContainer({ children, className = '' }: ContentContainerProps) {
    return (
        <div style={{ paddingBlockStart: '1.75rem' }} className={`max-w-6xl mx-auto space-y-4 leading-relaxed text-slate-800 [&_ul]:list-disc [&_ul]:pl-6 [&_ul]:space-y-2 ${className}`}>
            {children}
        </div>
    );
}
